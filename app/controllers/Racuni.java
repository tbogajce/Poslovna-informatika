...
public class Racuni extends Controller {
	...
	public static void edit(Racun racun/* , Long banka */, Long klijent, Long valuta) {
		...
		Klijent kl = Klijent.findById(klijent);
		Banka b = kl.getBanka(); /* Banka.findById(banka); */
		System.out.println("EDIT: " + racun.id + " OTVARANJE: " + racun.datumOtvaranja + " ZATVARANJE: "
				+ racun.datumZatvaranja + " STAT: " + racun.status);
	...
	public static void filter(Racun racun) {
		if (session.get("banka_id") != null) {
			List<Klijent> klijenti = Klijent.findAll();
			List<Valuta> valute = Valuta.findAll();
			List<Racun> r = Racun.find("byBrojRacunaLike", "%" + racun.brojRacuna + "%").fetch();
			List<Racun> racuni = new ArrayList<Racun>();
			for(int i=0; i<r.size();i++) {
				if(r.get(i).getBanka().getId().equals(Long.valueOf(session.get("banka_id"))))
					racuni.add(r.get(i));
			}
			String mode = "edit";
